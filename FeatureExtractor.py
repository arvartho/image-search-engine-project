...
#
   ...
   def _color_desc_vector(self, image=[]):
      ...
      if face_mask is not None:
         ...
         # area boundaries
         H, W = blured_image.shape[:2]
         x_padding = int(x*25/(W-x))
         y_padding = int(y*25/(H-y))
         # print('FACE DETECTED!')
         # area 1: face mask
         hist = self.histogram(blured_image, mask=face_mask, bins=bins)
         features.extend(hist)
         # area 2: face padding
         e_face_mask = np.zeros_like(image_bw)
         e_face_mask[y-y_padding : y+h+y_padding, x-x_padding : x+w+x_padding] = 1
         hist = self.histogram(blured_image, mask=e_face_mask, bins=bins)
         features.extend(hist)
         x_start, x_end, y_start, y_end = 0, W, y+h+y_padding, H
         ...
         x_end =  blured_image.shape[1] if x_end > blured_image.shape[1] else x_end
         y_end =  blured_image.shape[0] if y_end > blured_image.shape[0] else y_end
         area_mask = np.zeros_like(image_bw)
         area_mask[y_start:y_end, x_start:x_end] = 1
         hist = self.histogram(blured_image, mask=area_mask, bins=bins)
         features.extend(hist)
         bg_area = np.ones_like(image_bw) - area_mask -e_face_mask
         hist = self.histogram(blured_image, mask=bg_area, bins=bins)
         features.extend(hist)
         # self.cv_imshow(image)
         # self.cv_imshow(np.hstack((face_mask, e_face_mask, area_mask, bg_area)))

         # areas 3-6:
         # hist = self.histogram(image, mask=np.ones_like(image_bw)-e_face_mask, bins=bins)
         # features.extend(hist)
         ...
         #    area_mask = np.zeros_like(image_bw)
         #    area_mask[y_start:y_end, x_start:x_end] = 1
         #    hist = self.histogram(image, mask=area_mask, bins=bins)
         #    features.extend(hist)
      else:   
         (h, w) = image.shape[:2]      
